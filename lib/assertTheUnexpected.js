...
function assertTheUnexpected(expect) {
  ...
  expectWithLooseEquality.addAssertion('<any> to loosely equal <any>', function (expect, subject, expected) {
    var a = subject;
    var b = expected;
    ...
    function checkError(blockError) {
      if (isRegExp(errorConstraint)) {
        ...
        try {
          ...
          expect(errorConstraint(blockError), 'to be true').catch(function () {
            throw blockError;
          });
        }
      }
