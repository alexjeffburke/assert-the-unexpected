...
a.throws(makeBlock(thrower, a.AssertionError),
              a.AssertionError, 'message');
a.throws(makeBlock(thrower, a.AssertionError), a.AssertionError);
a.throws(makeBlock(thrower, a.AssertionError));

// if not passing an error, catch all.
a.throws(makeBlock(thrower, TypeError));
...
a.throws(function() {assert.ifError(new Error('test error'));});
a.doesNotThrow(function() {assert.ifError(null);});
a.doesNotThrow(function() {assert.ifError();});
...
// Make sure circular refs don't throw.
{
  ...
  assert.doesNotThrow(makeBlock(a.deepEqual, b, c));
  assert.doesNotThrow(makeBlock(a.deepStrictEqual, b, c));
  ...
  assert.throws(makeBlock(a.deepEqual, d, e), /AssertionError/);
  assert.throws(makeBlock(a.deepStrictEqual, d, e), /AssertionError/);
}
*/
// GH-7178. Ensure reflexivity of deepEqual with `arguments` objects.

var args = (function() { return arguments; })();
assert.throws(makeBlock(a.deepEqual, [], args));
assert.throws(makeBlock(a.deepEqual, args, []));
